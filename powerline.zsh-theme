...
if [ "$POWERLINE_DISABLE_RPROMPT" = "" ]; then
    if [ "$POWERLINE_RIGHT_A" = "" ]; then
        ...
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{$POWERLINE_RSEC1_COLOR}"$'\ue0b2'"%k%F{black}%K{$POWERLINE_RSEC1_COLOR} $POWERLINE_RIGHT_B %f%F{8}"$'\ue0b2'"%f%k%K{8}%F{white} $POWERLINE_RIGHT_A %f%k" 
    fi
fi
